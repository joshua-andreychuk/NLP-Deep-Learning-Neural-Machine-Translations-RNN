...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # Note: For the encoder we expect input shape (src_len, b, e) so we use the default batch_first=False.
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        ### START CODE HERE (~ 8 Lines)
        # Obtain embeddings for the source sentences.
        ...
        # Pad the packed sequence to obtain enc_hiddens of shape (src_len, b, 2h)
        enc_hiddens, _ = pad_packed_sequence(packed_enc_hiddens)
        # Transpose to shape (b, src_len, 2h)
        enc_hiddens = enc_hiddens.transpose(0, 1)
        # Concatenate the final hidden states from forward and backward directions and project.
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden, dec_cell = dec_state  # each of shape (b, h)
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(2)).squeeze(2)  # shape: (b, src_len)
        ### END CODE HERE
        ...
        U_t = torch.cat([dec_hidden, a_t], dim=1)  # shape: (b, h + 2h) = (b, 3h)
